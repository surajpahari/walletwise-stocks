            ...
            if 'application/json' in content_type:
                try:
                    ...
                    if destructured[-2] in ['security', 'graphdata']:
                        ...
                        json_file_name = os.path.join(file_dir, destructured[-2] + ".json")

                        with open(json_file_name, 'w') as file:
                            json.dump(json_data, file, indent=4)
                        logging.debug(f"Saved JSON response from {response.url} to {json_file_name}")

                        # Save graphdata as CSV if applicable
                        if destructured[-2] == 'graphdata':
                            csv_file_name = os.path.join(file_dir, destructured[-2] + ".csv")
                            with open(csv_file_name, 'w', newline='') as csvfile:
                                ...
                                for entry in json_data:
                                    csvwriter.writerow([
                                        ...
                                        entry["lowPrice"],
                                        entry["closePrice"],
                                        entry["totalTradedQuantity"]
                                    ])
                            logging.debug(f"Saved CSV response from {response.url} to {csv_file_name}")
